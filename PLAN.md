...
| **MV Verifier(Reader) SDK** | 양방향 SDK의 verifier 파사드. **mdoc reader 완료 (2026-07-05) 양 언어**: `MdocReader` — `buildDeviceRequest`(+readerAuth 서명), `verifyDeviceResponse`(issuerAuth 신뢰검증 + **deviceSignature over SessionTranscript holder 바인딩** + digest + validity). **reader↔wallet 풀 라운드트립**(reader 요청생성→wallet 응답→reader 검증) 양 언어 통과, 세션 바인딩·issuer 신뢰 강제. **잔여: OpenID4VP verifier**(authorization request 생성 + vp_token/KB-JWT 검증) | 3–5주(mdoc 완료) | ✅ mdoc reader E2E / ⬜ OpenID4VP verifier vp_token 검증 |
