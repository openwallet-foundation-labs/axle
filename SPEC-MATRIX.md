...
| iOS proximity transport (CoreBluetooth / CoreNFC) | ⬜ Android adapters only — the proximity protocol **and** session termination (status 20) are done in the portable core; only the iOS transport wiring is missing |
| NFC negotiated handover (18013-5 §8.2.2.1) | ✅ SDK (messages + `[Hs, Hr]` transcript + `present`/`read` wiring) **and** the Android on-wire transport (TNEP over a Type-4 HCE, auto-detected) — device-verified two-phone negotiated read |
