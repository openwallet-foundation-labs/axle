...
| **Verifier(Reader) SDK — mdoc** | dual-role: `MdocReader`가 DeviceRequest 생성+readerAuth 서명, DeviceResponse 검증(issuerAuth 신뢰+deviceSignature holder 바인딩+digest) | ✅ **완료 (2026-07-05) 양 언어** — `MdocReader.buildDeviceRequest`/`verifyDeviceResponse`, `DeviceResponse.decode`, `VerifiedDocument`. **reader↔wallet 풀 라운드트립**(요청생성→응답→검증) 통과, 세션바인딩·issuer신뢰 강제. 프리미티브 대칭(COSE·세션암호 양측)에 파사드만 얹음. **잔여: OpenID4VP verifier(request 생성+vp_token 검증)** |
| DeviceRequest + ReaderAuth (ISO 18013-5) | 리더 요청 파싱(docType·요소·intentToRetain), ReaderAuth(§9.1.4) 검증, org-iso-mdoc DC API(18013-7 Annex C) | ✅ **완료 (2026-07-05) 양 언어** — `DeviceRequest.decode`(version·docRequests·itemsRequest), `DocRequest.disclosable`(보유 요소와 교집합), `ReaderAuth.verify`(COSE_Sign1 over ReaderAuthentication=`["ReaderAuthentication",SessionTranscript,ItemsRequestBytes]` detached, 리더 인증서 체인→trust `X5cMdocReaderTrust`, **SessionTranscript 바인딩**), `MdocSessionTranscript.dcApiIsoMdoc`. **org-iso-mdoc DC API 라운드트립**(DeviceRequest→리더검증→공개→DeviceResponse) 양 언어 통과. Kotlin 5 + Swift 5 테스트 |
...
| HAIP 클라이언트 인증 (wallet attestation) | attestation 기반 client 인증 (OAuth 2.0 Attestation-Based Client Auth §4.2) — PAR·토큰 요청에 `OAuth-Client-Attestation` + `OAuth-Client-Attestation-PoP` | ✅ **완료 (2026-07-05) 양 언어** — `WalletClientAuth`(attestation JWT + `ClientAttestationPop`), VCI 클라이언트가 PAR+토큰에 두 헤더 첨부, PoP는 wallet 인스턴스 키로 서명(typ `oauth-client-attestation-pop+jwt`, iss=client_id, aud=AS issuer). 캡처 트랜스포트로 PAR·토큰 양쪽 헤더+PoP 서명 검증. client_id는 attestation subject |
| HAIP key attestation + batch 발급 | proof JWT 헤더의 `key_attestation`(OpenID4VCI §8.2.1.1), proofs 배열로 다중 크리덴셜 | ✅ **완료 (2026-07-05) 양 언어** — `KeyProofSigner.proofJwt(keyAttestation)`가 헤더에 `key_attestation` 첨부(`KeyAttestationSource` 주입, c_nonce 바인딩). `IssuanceKeys.additionalProofKeys`+`ProofKey`로 batch: 키마다 proof 생성→`proofs.jwt[]`, 이슈어가 키당 크리덴셜 1개 발급. 테스트: key_attestation이 proof 헤더 도달 확인, 3키→3proof→3개 distinct 크리덴셜 |
| HAIP tail: deferred + notification | deferred 발급(OpenID4VCI §9, transaction_id 폴링), 발급 notification(§10) | ✅ **완료 (2026-07-05) 양 언어** — `CredentialResponse.isDeferred`+`accessToken`/`credentialIssuer` 컨텍스트. `fetchDeferredCredential(deferred, keys)`가 deferred_credential_endpoint 폴링(issuance_pending→`VciException.IssuancePending`/`.issuancePending`). `sendNotification(response, event)`가 notification_endpoint에 {notification_id, event} POST(`NotificationEvent`: accepted/deleted/failure). MockIssuer(양 언어) deferMode+notification 핸들러. 테스트: 폴링 pending→ready, notification 수신 확인 |
| 크로스언어 골든 벡터 | 공유 `vectors/` 저장소를 Kotlin·Swift 테스트가 동시 소비 — 네이티브 2벌 바이트 정합성 잠금(PLAN §7 핵심 장치) | ✅ **4세트 완료 (2026-07-05)** — ①deterministic CBOR(RFC 8949 A.1 + §4.2 맵 정렬) ②COSE_Key EC2(P-256/384) ③SD-JWT disclosure digest(RFC 9901 + 스펙 권위 digest) ④DCQL 매칭(null-wildcard·values·nested 엣지케이스). 각 언어가 `vectors/*.json`을 walk-up으로 찾아 로드, 인코딩/digest/매칭결과가 골든과 일치. 어느 impl이 drift하면 공유 골든이 즉시 포착. 잔여(증분): 세션키 유도, JWE, mdoc 구조 벡터 |
| ARF | 2.7.x 추적 | 문서 단계 |
