...
screen, and verification-failure reporting. Five 🔴 conformance defects are recorded but **not yet fixed**
in the [18013-5 table](#isoiec-18013-52021) — `EReaderKeyBytes` re-encoding, per-`DocRequest` reader auth,
Annex B EKU, §9.3.3 IACA subject checks, and §9.3.1 steps 4-5. They sit
inside areas this matrix previously marked ✅, so read that table before trusting the "At a glance" row.
...
| Gap | Spec ref | Detail |
|---|---|---|
| `EReaderKeyBytes` re-created from the map | §8.1, §9.1.1.4 | 🔴 §8.1 is explicit: because canonical map ordering is not required, every CBOR map used in a cryptographic operation travels as a tagged bytestring and "an mdoc, mdoc reader or issuing authority infrastructure **shall use these bytestrings as they were sent or received, without attempting to re-create them from the underlying maps**". `SessionMessages.decodeEstablishment` decodes `EReaderKeyBytes` to an `EcPublicKey` and drops the received bytes (`SessionMessages.kt:38`, `SessionMessages.swift:27`), then `ProximitySessionTranscript.build` re-encodes them with `CoseKey.encode` (`ProximitySessionTranscript.kt:21`, `.swift:14`). `DeviceEngagementBytes`, `IssuerSignedItemBytes` and `ItemsRequestBytes` *are* carried verbatim — this is the one hole. Our COSE_Key encoding (`{1,-1,-2,-3}`, uncompressed) matches Multipaz's, so live interop passes; any peer that adds `kid`/`alg` or orders keys differently yields a different SessionTranscript and **session-key derivation fails outright** (and would break `DeviceAuth`/`ReaderAuth` too). Fix: carry the raw `EReaderKeyBytes` on `SessionEstablishment` and feed those bytes to the transcript |
...
| §9.3.1 inspection steps 4 and 5 | §9.3.1 | 🔴 two of the five mandated issuer-data-authentication steps are missing. **Step 4** — "verify that the DocType in the MSO matches the relevant DocType in the Documents structure": never compared (`MdocReader.kt:120-135`, `MdocReader.swift:125-140`); `DeviceAuthentication` is signed over the *Document's* docType while the app is handed the *MSO's*. **Step 5, first bullet** — "the 'signed' date is within the validity period of the certificate in the MSO header": `X509ChainValidator` always validates at `now()` (`X509ChainValidator.kt:48/64`, `.swift:60`), so an otherwise-valid mdoc whose DS certificate has since expired is rejected, while an MSO signed before the DS `notBefore` passes. `validFrom`/`validUntil` (bullets 2-3) *are* checked (`MdocVerifier.kt:54-56`, `.swift:48-50`) |
