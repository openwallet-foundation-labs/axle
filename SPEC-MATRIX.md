...
**2026-08-14** — ISO/IEC 18013-5:2021 re-read against the code (clauses 7.2.5, 8.1, 8.3.2, 9.1, 9.3,
Annex B). Closed in this pass: §7.2.5 age attestation resolution, `IntentToRetain` reaching the consent
screen, verification-failure reporting, `EReaderKeyBytes` binding (§8.1), and §9.3.1 steps 4-5. Three 🔴
conformance defects remain recorded and **not yet fixed** in the [18013-5 table](#isoiec-18013-52021) —
per-`DocRequest` reader auth, Annex B EKU, and §9.3.3 IACA subject checks. They sit inside areas this
matrix previously marked ✅, so read that table before trusting the "At a glance" row.

Legend: ✅ implemented · 🟡 partial · ⬜ not yet · 🔴 conformance defect (a normative *shall* the code does not meet).
...
| Client ID prefixes `verifier_attestation` / `decentralized_identifier` / `openid_federation` | §5.9.3/§12 | ⬜ **parsed** — all six §5.9.3 prefixes are recognised, and per §5.9.2 an unprefixed identifier (or one whose head is not a defined prefix, e.g. the `https` of a bare Redirect URI) resolves to `pre-registered`. **Verified** for `x509_san_dns` / `x509_hash`, and `redirect_uri` is enforced per §5.9.3 — the Client Identifier must equal the `response_uri` (`direct_post`) or `redirect_uri`, it supplies that endpoint when the Verifier omits it, and a signed request under this prefix is rejected ("cannot be signed because there is no method for the Wallet to obtain a trusted key"). The remaining three reach `X509RequestVerifier` and are rejected as `Unsupported` |
...
| Gap | Spec ref | Detail |
|---|---|---|
| Reader auth verified for one `DocRequest` only | §9.1.4 | 🔴 `readerAuth` signs `ReaderAuthentication = ["ReaderAuthentication", SessionTranscript, ItemsRequestBytes]` — its *own* `ItemsRequestBytes` — so a multi-document `DeviceRequest` carries one signature per `DocRequest`, and §9.1.4 requires the mdoc to regenerate the structure and validate the signature. `ProximityService.verifyReader` validates only the first `DocRequest` that has a `readerAuth` (`ProximityService.kt:223`, `.swift:177`) and applies that verdict to the whole request, so documents/elements requested in unsigned `DocRequest`s are presented to the holder under a "trusted reader" badge. Needs per-document verification and a per-document trust flag on `RequestedDocumentView` |
| Annex B certificate profile (EKU) not enforced | §6.3.2.1, Annex B.1.4/B.1.7 | 🔴 "The certificate and CRL profile requirements in Annex B **shall** be applied." Annex B makes `extendedKeyUsage` mandatory **and critical**: `1.0.18013.5.1.2` (mdlDS) on the Document Signer, `1.0.18013.5.1.6` (mdlReaderAuth) on the reader-auth certificate. Neither tree checks the OID — Swift only *accepts* a critical EKU so the chain validates at all (`AcceptExtendedKeyUsagePolicy`, `X509ChainValidator.swift:78`, whose own comment defers enforcement to the caller), Kotlin runs plain PKIX (`X509ChainValidator.kt:62`). Any end-entity cert under the same anchor therefore passes as a DS or as a reader. §9.3.3 additionally requires access to revocation information; `X509ChainValidator(checkRevocation = false)` is the default in both trees |
| §9.3.3 IACA subject checks | §9.3.3 | 🔴 RFC 5280 §6.1 basic path validation runs, but not the two steps §9.3.3 adds for certificates issued by an IACA: the `countryName` of the IACA subject and of the target certificate **shall** be the same, and `stateOrProvinceName` **shall** be the same when present in both. Absent from both `X509ChainValidator`s — a US-state or cross-country mismatch validates today |
| §9.3.1 inspection steps 4 and 5 | §9.3.1 | ✅ the two mandated issuer-data-authentication steps that were missing are now enforced. **Step 4** — "verify that the DocType in the MSO matches the relevant DocType in the Documents structure": `MdocReader.verifyDeviceResponse` rejects a mismatch, which previously let `DeviceAuthentication` be signed over the *Document's* docType while the caller was handed the *MSO's*. **Step 5, first bullet** — "the 'signed' date is within the validity period of the certificate in the MSO header": `MdocIssuerTrust` now returns an `MdocIssuerKey` carrying the Document Signer's `notBefore`/`notAfter`, and `MdocVerifier` checks `signed` against it. This is a check on the *signing moment*, deliberately separate from step 1's chain validation (which stays at the verifier's clock), so a credential signed while its DS was valid keeps verifying after that certificate expires. `validFrom`/`validUntil` (bullets 2-3) were already checked |
| Age attestation: nearest "true" above request | §7.2.5 | ✅ `AgeAttestation` (both trees) implements the three-step rule literally — nearest TRUE at/above the request, else nearest FALSE at/below it, else return nothing — and `DocRequest.disclosable` resolves `age_over_NN` through it instead of matching literally, so `age_over_18` is answered by a held `age_over_21: true` (and *not* by `age_over_16: true`). Resolution stays inside the requested namespace. The reader half of the clause — "an mDL reader shall not request more than two age_over_NN data elements" — is enforced per document in `buildDeviceRequest`. Applies to proximity **and** the 18013-7 `org-iso-mdoc` DC API path, which carries a real `DeviceRequest`; deliberately *not* applied to OpenID4VP DCQL, where substituting a claim the verifier did not query would fail its own claims matching |
| `IntentToRetain` reaches the consent screen | §8.3.2.1.2.1, Annex E | ✅ the flag was parsed and then dropped: `RequestedDocumentView` exposed identifiers only and the raw `DeviceRequest` is internal, so a host app could not see it at all. `DocRequest.retained()` now surfaces it and `RequestedDocumentView.retainedElements` carries it into both consent paths (proximity + DC API). The reader side can also set it **per element** (`RequestedDocument.retainedElements`), the granularity the CDDL defines — `intentToRetain` remains the per-document default. The normative duty is the verifier's ("shall not retain any data … except for data elements for which the IntentToRetain flag was set to true"); showing it at consent time is Annex E ("Consent applies to both selective disclosure and authorization for intent-to-retain") |
| Verification failure reporting | §9.3.1 | ✅ `ProximityReaderService` used to catch every verification failure and return the disclosed elements with a bare `deviceAuthenticated = false`, discarding the exception — so "no trust anchor configured", "issuerAuth signature invalid" and "digest mismatch" were indistinguishable to the caller, and the §9.3.1 findings that are forgery signals looked like configuration gaps. `VerifiedDocument.verificationError` now carries the reason from the point of failure (both trees) |
| `EReaderKeyBytes` bound as received | §8.1, §9.1.1.4 | ✅ §8.1: because canonical map ordering is not required, every CBOR map used in a cryptographic operation travels as a tagged bytestring and "an mdoc, mdoc reader or issuing authority infrastructure **shall use these bytestrings as they were sent or received, without attempting to re-create them from the underlying maps**". `SessionEstablishment` now carries the received `eReaderKeyBytes` and `ProximitySessionTranscript.build` binds those bytes; the reader encodes its key once (`SessionMessages.eReaderKeyBytes`) and reuses it for both the frame and the transcript. Previously the holder re-encoded the parsed COSE_Key, which matches our own and Multipaz's encoding but silently derives a different SessionTranscript — and therefore different session keys — against any peer that orders the map differently or adds `kid`. `DeviceEngagementBytes`, `IssuerSignedItemBytes` and `ItemsRequestBytes` were already verbatim |
...
| **Multi-signed DC API requests** (`openid4vp-v1-multisigned`) | OpenID4VP A.3.2.2 | JWS JSON Serialization, one signature per Client Identifier, so one request authenticates under several trust frameworks at once. Verifiers SHOULD use Compact Serialization and every verifier we interoperate with does; a wallet serving one ecosystem gains nothing from the second signature. It is refused explicitly — by protocol identifier *and* by the `{payload, signatures}` shape — and not declared at registration, so the platform never routes one here. Pinned by `DcApiProtocolGuardTest` / `DcApiProtocolGuardTests` |
