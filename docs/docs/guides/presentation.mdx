...
`PresentationSelection.auto(request)` picks the first candidate for each required query — ideal once the
user has approved a single obvious match. For a real consent UI, build the selection explicitly by
mapping each `queryId` to the `credentialId` the user chose:
