...
binding; without them the reader still returns the disclosed elements, marked unverified.
...
`ProximityDeviceAuth.Mac` requires a key-agreement `DeviceKey`. On Android Keystore and the Secure
...
Only the proximity path has an `EReaderKey`, so **the DC API (`org-iso-mdoc`) and OpenID4VP mdoc paths
always sign**, whatever `proximityDeviceAuth` says. With the default `KeyUse.Rotate` policy the same
DeviceKey is reused, so a wallet set to `Mac` that also presents over those paths makes one key do both
— which the clause forbids, and which quietly undoes the deniability that `Mac` was chosen for.
