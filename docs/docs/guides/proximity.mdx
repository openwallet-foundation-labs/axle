...
- **Device engagement** — a **QR code** or **NFC handover** — static or negotiated (Handover Select over HCE), either
...
choosing exactly one of them. Pass those Request bytes to `present(transport, nfc = true,
handoverRequestNdef = hr)` and the SDK binds `[Hs, Hr]` into the SessionTranscript (static binds
...
results live in [`demo/PROXIMITY-MATRIX.md`](https://github.com/openwallet-foundation-labs/axle/blob/main/demo/PROXIMITY-MATRIX.md).
